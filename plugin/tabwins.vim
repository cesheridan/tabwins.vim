...
" --------------------------------------------------------------- 
function! Build_cmds_for_symmetric_tabs(args_hash)
" --------------------------------------------------------------- 
  let l:args = extend({
\   'primary_dim' : 'V',
\   },
...
\ )
  ...
  elseif(l:args['primary_dim'] == 'H')
    ...
    call Report_error({ 'message' :
    \ "ERROR: Build_cmds_for_symmetric_tabs() Unrecognized 'primary_dim' = " .
    \ l:args['primary_dim']
    \})
    return 0
  endif
    ...
    for l:count2 in range(1, l:secondary_dim_max)
     ...
     "\  "\'\. <q-args> ". "\.' })"
      ...
      if (l:execute_arg =~ 'x1 ') && (l:count1 <= 10)
        ...
        " represents symmetric grids.  So, use V{I...} namming format -- e.g. V123
...
\   deepcopy(a:args_hash,1)
\ )
"echo 'Tabwins_create_tab' echo l:args
...
" echo  l:fill_specs
" echo  l:fill_specs
" echo  type(l:tab_options)
  call Tabwins_create_tab({
\   'primary_dim' : l:args['primary_dim'],
\   'dims'        : l:dims,
...
\ :call  Build_cmds_for_symmetric_tabs({'primary_dim' : 'V'})

command! BuildCmdsForSymmetricTabsH
\ :call  Build_cmds_for_symmetric_tabs({'primary_dim' : 'H'})

" Test case for failure
command! BuildCmdsForSymmetricTabsERROR
\ :call  Build_cmds_for_symmetric_tabs({'primary_dim' : 'X'})
" --------------------------------------------------------------- 
...
" asymmetric tab.  So tabwins v2.0.0 removes the asymmetric tab 
...
" this works if these files are in &runtimepath -- including ~/.vim 
